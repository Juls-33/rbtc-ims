...
export default function EditRoomModal({ isOpen, onClose, room, onSuccess }) {
    ...
    const submit = (e) => {
        ...
        if (validate()) {
            put(route('admin.rooms.update', room.id), {
                onSuccess: () => {
                    setToastInfo({ show: true, message: 'Room details updated!', type: 'success' });
                ...
                onError: (err) => {
                    setToastInfo({ 
                        show: true, 
                        message: 'Update failed. Check inputs.', 
                        type: 'error' 
                    });
                }
            });
    ...
    const inputClass = (error) => `w-full border rounded px-3 py-2 text-sm transition-colors ${
        error ? 'border-red-500 bg-red-50 focus:ring-red-500 focus:border-red-500' : 'border-slate-300 focus:ring-[#E6AA68] focus:border-[#E6AA68]'
    }`;
            ...
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-md flex flex-col max-h-[95vh] md:max-h-[90vh] overflow-hidden animate-in zoom-in duration-150">
                ...
                <div className="bg-[#E6AA68] text-[#5c3a00] px-6 py-4 flex justify-between items-center shrink-0 shadow-sm">
                    <div>
                        ...
                        <p className="text-[10px] text-amber-900 uppercase font-bold mt-1 tracking-widest opacity-70">Updating Facility ID: {room?.id}</p>
                    </div>
                    <button onClick={handleModalClose} className="text-2xl font-bold leading-none hover:text-black transition-colors">&times;</button>
                </div>
                ...
                <form onSubmit={submit} className="flex flex-col overflow-hidden">
                    <div className="p-6 md:p-8 space-y-6 overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-slate-300">
                        ...
                        <div>
                            <Label text="Room Location / Name" fieldError={errors.room_location} />
                            <input 
                                type="text" 
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    type="number" 
                                    step="0.01"
                                    className={`${inputClass(errors.room_rate)} pl-7 font-mono font-bold text-emerald-700`}
                        ...
                        <div>
                            <Label text="Operational Status" required={false} />
                            <select 
