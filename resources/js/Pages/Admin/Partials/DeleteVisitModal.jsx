...
export default function DeleteVisitModal({ isOpen, onClose, visitId, onSuccess }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        destroy(route('admin.visits.destroy', visitId), {
            onSuccess: () => {
                if (onSuccess) onSuccess('Visit record archived successfully.');
                handleClose();
            },
            preserveScroll: true,
        });
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-rose-100 transform animate-in zoom-in duration-150">
                ...
                <div className="bg-rose-600 text-white p-5 flex justify-between items-center shadow-lg">
                    <div>
                        <h3 className="font-black text-lg uppercase tracking-tight">Delete Visit</h3>
                ...
                <form onSubmit={submit} className="p-6 space-y-5 text-slate-800">
                    <div className="bg-rose-50 border border-rose-100 p-3 rounded-lg">
                        <p className="text-[11px] text-rose-700 leading-relaxed font-medium">
                            <strong>Warning:</strong> This will archive the visit record and return any medicines added back to the inventory stock. This action is tracked in audit logs.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea 
                            ...
                            placeholder="e.g., Wrong patient selected, Duplicate entry..."
                        />
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={handleClose} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg font-black text-[11px] uppercase tracking-widest transition-colors">
                            Cancel
                        </button>
                        <button 
                            type="submit" 
