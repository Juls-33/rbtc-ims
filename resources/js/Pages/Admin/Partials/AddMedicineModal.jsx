...
export default function AddMedicineModal({ isOpen, onClose }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (validate()) {
            post(route('inventory.store'), {
                onSuccess: () => {
                    setToastInfo({ show: true, message: 'New medicine added to inventory!', type: 'success' });
                    handleModalClose();
                },
                onError: () => {
                    setToastInfo({ show: true, message: 'Creation failed. Please check the fields.', type: 'error' });
                }
            });
        }
    };
    ...
    const inputClass = (error) => `w-full border rounded px-3 py-2.5 text-sm transition-colors ${
        error ? 'border-red-500 focus:ring-red-500 bg-red-50' : 'border-slate-300 focus:ring-[#2E4696] focus:border-[#2E4696]'
    }`;
