...
export default function EditPatientModal({ isOpen, onClose, patient }) {
    ...
    const submit = (e) => {
        ...
        if (validate()) {
            put(route('admin.patients.update', patient.id), {
                onSuccess: () => {
                    setToastInfo({ show: true, message: 'Patient Record Updated!', type: 'success' });
                    handleModalClose(); 
                },
                onError: () => {
    ...
    return (
        <>
            {/* Toast remains visible after modal closes */}
            {toastInfo.show && (
                <Toast 
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="bg-[#3D52A0] text-white p-4 flex justify-between items-center shadow-md">
                            <div>
                                <h3 className="font-bold text-lg leading-none uppercase tracking-tight">Edit Patient Record</h3>
                                <p className="text-[10px] text-blue-100 uppercase tracking-widest mt-1">Updating ID: {patient?.patient_id || patient?.id_no || patient?.id || '---'}</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" maxLength={100} value={data.first_name} onChange={e => setData('first_name', e.target.value)} className={inputClass(errors.first_name)} placeholder="Juan" />
                                    ...
                                    <div>
                                        ...
                                        <input type="text" maxLength={50} value={data.last_name} onChange={e => setData('last_name', e.target.value)} className={inputClass(errors.last_name)} placeholder="Dela Cruz" />
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <div className="flex justify-between items-center mb-1">
                                            ...
                                            {agePreview && <span className="text-[9px] font-bold bg-blue-100 text-blue-700 px-2 rounded-full animate-in zoom-in-50">{agePreview}</span>}
                                        </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" maxLength={200} value={data.address} onChange={e => setData('address', e.target.value)} className={inputClass(errors.address)} placeholder="Street, Brgy, City" />
                            ...
                            <div className="space-y-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" maxLength={150} value={data.emergency_contact_name} onChange={e => setData('emergency_contact_name', e.target.value)} className={inputClass()} placeholder="Maria Clara" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <Label text="Relationship" current={data.emergency_contact_relation.length} max={50} />
                                            <input type="text" maxLength={50} value={data.emergency_contact_relation} onChange={e => setData('emergency_contact_relation', e.target.value)} className={inputClass()} placeholder="Spouse" />
                                        </div>
                                        <div>
