...
export default function NurseFaqs({ auth }) {
    const [activeTab, setActiveTab] = useState('workflow');

    const tabs = [
        { id: 'workflow', name: 'Administration Workflow', icon: medicineIcon },
        { id: 'vitals', name: 'Patient Vitals', icon: patientIcon },
    ...
    return (
        <AuthenticatedLayout 
            auth={auth} 
            header="Nurse User Guide" 
            sectionTitle="Nursing Manual"
        >
            <Head title="Nurse Guide" />
                ...
                <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm p-6 md:p-8 min-h-[50vh]">
                    {activeTab === 'workflow' && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
                            <h3 className="text-xl font-black text-[#2E4696] uppercase mb-4">Medicine Administration</h3>
                            <FaqItem q="How do I handle 'Overdue' medications?" a="The dashboard highlights doses that are past their scheduled time. Click 'Administer' immediately to record the dose and deduct inventory." />
                            <FaqItem q="What is 'Outside Procured' medicine?" a="Use this for medications the patient brought from home. It logs the administration for medical history but adds a ₱0 charge to the bill." />
                            <FaqItem q="Why are some batches missing?" a="The system only shows batches that are in-stock and not expired. Contact the Admin if a supply is missing from the list." />
                        </div>
                    )}
                    {activeTab === 'vitals' && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
                            <h3 className="text-xl font-black text-[#2E4696] uppercase mb-4">Vitals Tracking</h3>
                            <FaqItem q="How often should I update vitals?" a="Updates should follow hospital protocol. Clicking 'Update Vitals' saves a timestamped record that Doctors use to monitor patient progress." />
                            <FaqItem q="Can I delete a vitals record?" a="Nurses can only delete vitals records that they personally created. Records created by other staff are locked for security." />
                        </div>
                    )}
                    {activeTab === 'account' && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
                            <h3 className="text-xl font-black text-[#2E4696] uppercase mb-4">Account Management</h3>
                            <FaqItem q="What should I do if I forget my password?" a="Contact your System Administrator. They can perform a 'Quick Reset' which allows you to log in and choose a new password." />
                            <FaqItem q="How do I update my profile?" a="Click your profile name in the top right header and select 'Profile Settings' to change your personal information or security settings." />
                        </div>
                    )}
                </div>
            </div>
